...
public:
    int knightDialer(int n)
    {
        ...
        if (n == 1)
            return 10;
        for (int movesLeft = 2; movesLeft <= n; ++movesLeft)
        {
            for (int currentDigit = 0; currentDigit < 10; ++currentDigit)
            {
                for (int nextDigit : moves[currentDigit])
                    dp[currentDigit][movesLeft] = (dp[currentDigit][movesLeft] + dp[nextDigit][movesLeft - 1]) % 1000000007;
            }
        }
        int totalWays = 0;
        for (int i = 0; i < 10; ++i)
            totalWays = (totalWays + dp[i][n]) % 1000000007; 
        return totalWays;
    }
};